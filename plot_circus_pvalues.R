...
#Family is the type of variable, could be null
#labels is the label of each variable
#pvalues are the pvalues
plot_circus_pvalues<-function(family,labels,pvalues, guides=c(5,20,50))
{
library(plyr)
library(ggplot2)
library(Cairo)
  
guides    = sort(guides,decreasing = F)
pvalues1  = -log10(pvalues)
maxP      = ceiling(max(pvalues1));
pvalues   = 1 - (pvalues1/maxP)
myfunction <- function(x) {pvalues[abs(pvalues1-x) == min(abs(pvalues1-x))]*100}
newguides    = vapply(-log10(guides),myfunction,2.3)
myguides  = signif(-log10(guides),2);

if(T){ #Use Max Y = 1
  df1=data.frame(
  family=as.vector(rbind(family, family)),
  item=as.vector(rbind(labels, labels)),
  score=as.vector(rbind(rep("V1",length(pvalues)), rep("V2",length(pvalues)))),
  value=as.vector(rbind(pvalues, 1)),
  previous=as.vector(rbind(0,pvalues)),
  indexItem=as.vector(rbind(1:length(pvalues),1:length(pvalues))),
  indexFamily= match(as.vector(rbind(family, family)),unique(family))
  )  
} else{ #Use Max Y from pvalues
  df1=data.frame(
    family=as.vector(rbind(family, family)),
    item=as.vector(rbind(labels, labels)),
    score=as.vector(rbind(rep("V1",length(pvalues)), rep("V2",length(pvalues)))),
    value=as.vector(rbind(pvalues, ceiling(max(pvalues)))),
    previous=as.vector(rbind(0,pvalues)),
    indexItem=as.vector(rbind(1:length(pvalues),1:length(pvalues))),
    indexFamily= match(as.vector(rbind(family, family)),unique(family))
  )  
  ...
  p<-polarHistogram(df1,familyLabel=F,guides=newguides,circleProportion=0.80,guidelabels=myguides)
...
test_circus_plot <- function()
{
  ...
  CairoPDF("circus_plot_new.pdf", 10, 10, bg="transparent")
  print(myP)
  dev.off()
}
...
#   if(  max(df$value) > 1 ) {
#     #outerRadius = ceiling(max(df$value))  
#     binSize     = outerRadius
#   }
#   
  print(max(df$value))
...
#              )
  ...
  maxY=ceiling(max(df$value))
  if(maxY<1){
    maxY=1
  } 
  
  print(sprintf("CircosPlot: Setting maxY=%d\n",maxY))
  
  affine<-switch(direction,
                 'inwards' = function(y) (outerRadius-innerRadius)*y+innerRadius,
                 'outwards'= function(y) (outerRadius-innerRadius)*(maxY-y)+innerRadius,
                 stop(paste("Unknown direction")))
  ...
  df<-within(df,{
    xmin<-(indexItem-1)*binSize+(indexItem-1)*spaceItem+(indexFamily-1)*(spaceFamily-spaceItem)
    xmax<-xmin+binSize
    ymin<-affine(maxY - previous)
    ymax<-affine(maxY - value)
  }
  )
...
if(ALX){
  dfItemLabels<-df[match(unique(df$indexItem),df$indexItem),c("item","xmin")]
}else{
...
if(T){
  ...
  if(T){
    ...
    guideLabels<-data.frame(
      ...
      label=paste(guidelabels," ",sep='')
      )  
  }else{
    # label for guides
    guideLabels<-data.frame(
      x=0,
      y=affine(maxY-guides/100),
